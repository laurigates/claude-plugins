...
|---|---|---|---|---|---|---|
| `blueprint-plugin/hooks/blueprint-structural-cue.sh` | `blueprint-plugin` | `PostToolUse` | `Edit\|Write` | `decision:block` + `continueOnBlock` | `~/.cache/blueprint-structural-cue/<session_id>` | Manifest edit (`plugin.json`, `marketplace.json`), public-symbol/export line, TS `export interface/type`, exported Go/Rust types, route registration, or schema/IDL files (`*.proto`, `*.graphql`, `openapi*`) — widened by #1616. **The payload-grep signals require a source extension** (`js jsx mjs cjs ts tsx mts cts rs py pyi go`) — without that guard prose containing "export " fired the public-API cue (#2336); the path/basename signals stay extension-free. Excludes `docs/adrs/**`, `docs/prds/**`, and the harness session scratch tree (`{/tmp,/private/tmp,/var/folders/*}/claude-*/…`). Bypass: `BLUEPRINT_SKIP_HOOKS=1`. |
...
| `code-quality-plugin/hooks/code-quality-preflight-cue.sh` | `code-quality-plugin` | `PostToolUse` | `Edit\|Write` | `decision:block` + `continueOnBlock` | `~/.cache/code-quality-preflight-cue/<session_id>` | Public-symbol line, manifest (`plugin.json`, `marketplace.json`, `package.json`, `Cargo.toml`, `pyproject.toml`), or payload ≥50 lines; excludes docs/tests/lockfiles. **Second suppression layer — sequence debounce (#2272):** silent when another Edit/Write to the **same file** landed within `CODE_QUALITY_PREFLIGHT_CUE_DEBOUNCE_TTL` seconds (default 120; `0` disables). Recency markers live under `<cache-dir>/.edits/<session_id>/<file-key>` — a *second* cache path this hook writes, distinct from the dedup key on the left — and a debounced edit **does not consume** the once-per-session budget, so the session's one cue lands on a settled edit. **Cache hygiene:** a *third* artifact, `<cache-dir>/.last-sweep`, gates an at-most-hourly prune of markers untouched for `CODE_QUALITY_PREFLIGHT_CUE_MARKER_TTL` minutes (default 1440); unlike the debounce knob, `0` is **rejected** rather than treated as "disable" (it would mean "prune anything a minute old" and disarm live debounces). Both layers fail open, as does cache-path resolution — an unset `HOME` falls back to `${TMPDIR:-/tmp}` rather than aborting the tool call. |
