...
| Missing `## ` section | WARN | Blocking an edit until someone back-writes a section the ADR never had is hostile. ADR-0023 narrowed the required set to the three load-bearing sections, taking the corpus from 45 section warnings to 5; the other two are convention, unpoliced |
...
**Channel:** `{"decision":"block","reason":"[blueprint] …"}` with `"continueOnBlock": true` on both `hooks.json` registrations, so the model receives the cue and the turn continues. This is the same channel `code-quality-preflight-cue.sh` uses for the same event. It previously used `hookSpecificOutput.updatedToolOutput`, which is the wrong mechanism here: that field is validated against the tool's own output shape, and Write/Edit's shape (`{content, filePath, originalFile, structuredPatch, type, userModified}`) has no free-text field — `content` is the file's actual content, so a cue banner there would make the model believe the banner was written to disk. Emitting a string instead was rejected outright and silently discarded, making the hook a no-op (issue #2275). The `continueOnBlock` flag is load-bearing: without it a `decision:block` would **end the turn**.

Regression test: `hooks/test-blueprint-structural-cue.sh` (run with `bash blueprint-plugin/hooks/test-blueprint-structural-cue.sh`, or via `scripts/run-skill-script-tests.sh`, which discovers it automatically). It replaces the former `hooks/spec/blueprint_structural_cue_spec.sh` — that ShellSpec suite pinned the broken output shape and was never executed by CI, pre-commit, or any recipe.
