...
All documents live at the **top level** of `docs/` — not under `docs/blueprint/`. `docs/blueprint/` holds blueprint machinery (manifest, feature-tracker, work-orders, ai_docs) only.

| Document Type | Directory | Naming Convention | Purpose | Generated By |
|--------------|-----------|-------------------|---------|---|
| PRD | `docs/prds/` | `{feature-name}.md` | Product Requirements Documents | `/blueprint:derive-prd` |
| ADR | `docs/adrs/` | `{number}-{title}.md` | Architecture Decision Records | `/blueprint:derive-adr` |
