...
| `taskwarrior-plugin` skills prescribe hyphenated tag names (`+blocked-on-merge`, `+pr-ready`, `+needs-review`, `+bulk-task`) that taskwarrior silently parses as `+word AND -rest` (exclude-filter), so the tag never lands and the literal string ends up appended to the description. Quoting does not help — taskwarrior parser quirk. | Recommended-tag tables and example lists across `task-add`, `task-status`, `task-coordinate`, README all used hyphens. Underscore (`+blocked_on_merge`) or camelCase (`+blockedOnMerge`) variants work correctly. | `plugin-compliance-check.sh` `check_skill_body()` flags hyphenated taskwarrior tag names appearing in table rows or list items in `taskwarrior-plugin` SKILL.md files (allows prose mentions inside the gotcha callout) | issue #1237 |
