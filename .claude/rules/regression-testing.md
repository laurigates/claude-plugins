...
| `taskwarrior-plugin` skills documented `+pr-ready`, `+needs-review`, `+blocked-on-merge`, `+bulk-task` as if they worked | Hyphens in taskwarrior tag names silently break parsing: `+blocked-on-merge` is parsed as `+blocked` AND `-on-merge` (mid-token `-` triggers exclude-filter syntax), so the tag is never applied and the literal token ends up in the description. Quoting does not help — taskwarrior parser quirk, not shell. Affected `task-add`, `task-status`, `task-coordinate`, and the plugin README. | `scripts/check-taskwarrior-tag-syntax.sh` (scans taskwarrior-plugin SKILL.md / README for hyphenated tag examples; skips intentional gotcha callouts) | issue #1237, PR #TBD |
