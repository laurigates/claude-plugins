...
| Hyphenated `+tag-name` examples in taskwarrior skills are silently dropped by the parser | Taskwarrior parses `-` as the exclude-filter operator even mid-token, so `+blocked-on-merge` is read as `+blocked` AND `-on-merge` (exclude). Quoting does not help. Affected `task-add`, `task-status`, `task-coordinate`, plus the README tag table — users following the docs ended up with `tags: null` and the literal tokens appended to the description. | `plugin-compliance-check.sh` `check_skill_body()` flags `+word-word` examples in `taskwarrior-plugin/skills/**/SKILL.md` outside the gotcha callout | issue #1237 |
