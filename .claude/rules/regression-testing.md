...
| Hyphens in taskwarrior tag examples (`+blocked-on-merge`, `+pr-ready`, `+needs-review`, `+bulk-task`) silently break parsing | Taskwarrior parses `+blocked-on-merge` as `+blocked` AND `-on-merge` (the `-` is its exclude-filter syntax, even mid-token). Tags get swallowed into description as plain text, the `tags` array stays null, and urgency does not tick up. Quoting does not help. Underscores or camelCase work fine. Affected `task-add`, `task-status`, `task-coordinate` SKILL.md plus `taskwarrior-plugin/README.md`. | `plugin-compliance-check.sh` `check_skill_body()` flags any `+\w+-\w+` pattern in `taskwarrior-plugin` skills (except `task-add/SKILL.md`, which keeps the documented gotcha callout) and asserts the `Tag naming gotcha` section exists in `task-add/SKILL.md` | issue #1237 |
