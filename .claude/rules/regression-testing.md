...
| `taskwarrior-plugin` documented hyphenated tag examples (`+blocked-on-merge`, `+pr-ready`, `+needs-review`) that taskwarrior silently breaks at parse time | Hyphens mid-token are interpreted as exclude-filter syntax: `+blocked-on-merge` parses as `+blocked` AND `-on-merge`. The tag never lands; the literal token is appended to the description as plain text. Quoting (`'+blocked-on-merge'`) does not help — this is a taskwarrior parser quirk, not a shell issue. All four taskwarrior skills' tag tables and the README used hyphenated examples, so users following the docs filed broken tasks. | `plugin-compliance-check.sh` `check_skill_body()` (taskwarrior-plugin-only check rejects `+word-word` patterns; ignores explanatory callouts that mention "hyphen", "broken", "silently", "parsed as", "does not help", or "never lands") | issue #1237 |
