...
| Hyphenated taskwarrior tags (`+blocked-on-merge`, `+pr-ready`, `+needs-review`, `+bulk-task`) silently fail to apply — they parse as `+foo` AND `-bar` (taskwarrior's exclude-filter syntax even mid-token), so the literal token is appended to the description with `tags: null`. Quoting does not help. Affected `task-add`, `task-status`, `task-coordinate`, and the plugin README. Skill examples taught users patterns that don't actually work. | `plugin-compliance-check.sh` `check_skill_body()` (taskwarrior-plugin scoped check rejects `+<word>-<word>` tag patterns; explanatory gotcha lines excluded by keyword filter) | issue #1237 |
