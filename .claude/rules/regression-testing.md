...
| `taskwarrior-plugin` SKILLs promoted hyphenated tag examples (`+blocked-on-merge`, `+pr-ready`, `+needs-review`, `+bulk-task`); taskwarrior parses `+blocked-on-merge` as `+blocked` AND `-on-merge` (exclude filter), silently dropping the tag and appending the literal token to the description with `tags: null` and no urgency tick | `task-add`, `task-status`, `task-coordinate` SKILLs and `taskwarrior-plugin/README.md` documented hyphenated forms as the canonical convention. `-` is taskwarrior's exclude-filter syntax even mid-token; quoting (`'+blocked-on-merge'`) does not help — it is a parser quirk, not a shell issue. | `plugin-compliance-check.sh` `check_skill_body()` flags hyphenated `` `+xxx-yyy` `` examples in `taskwarrior-plugin` SKILLs unless the same line carries gotcha-context keywords (`swallow`, `silently break`, `hyphens`, `gotcha`, `parser quirk`, `does not help`, `exclude filter`) | issue #1237 |
