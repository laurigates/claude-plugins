...
def js_export_name(plugin_name):
    return (
        "".join(p.capitalize() for p in re.split(r"[^A-Za-z0-9]+", plugin_name) if p)
        + "Hooks"
    )
    ...
    for event, groups in (manifest.get("hooks") or {}).items():
        ...
        for group in groups:
            ...
            for hook in group.get("hooks", []):
                ...
                if slot is None:
                    skipped.append(
                        (event, matcher, kind, "no OpenCode equivalent for this event")
                    )
                    continue
                if kind != "command":
                    skipped.append(
                        (event, matcher, kind, "OpenCode has no model-evaluation hook")
                    )
                    continue
...
def main():
    ...
    plugin_dirs = sorted(p.parent for p in repo_root.glob("*-plugin/hooks.json"))
