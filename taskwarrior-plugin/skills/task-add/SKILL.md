...
**Hyphens silently break tag parsing.** `+blocked-on-merge` is parsed as `+blocked` AND `-on-merge` (taskwarrior treats a `-` mid-token as exclude-filter syntax), so the tag never lands and the literal `+blocked-on-merge` ends up appended to the description as plain text. Quoting (`'+blocked-on-merge'`) does not help — this is a taskwarrior parser quirk, not a shell issue.

Use underscores or camelCase: `+blocked_on_merge`, `+blockedOnMerge`. All tag examples in this plugin use the underscore form for this reason.
