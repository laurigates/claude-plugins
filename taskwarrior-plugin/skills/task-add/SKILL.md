...
> **Tag naming gotcha.** Hyphens silently break taskwarrior tag parsing — `+blocked-on-merge` is parsed as `+blocked` AND `-on-merge` (the `-` is taskwarrior's exclude-filter syntax mid-token), so the tag is never applied and the literal token ends up appended to the description. Quoting (`'+blocked-on-merge'`) does not help — this is a taskwarrior parser quirk, not a shell issue. Use underscores (`+blocked_on_merge`) or camelCase (`+blockedOnMerge`).
