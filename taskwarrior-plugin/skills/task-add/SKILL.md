...
**Hyphens break tags.** `+blocked-on-merge` is parsed as `+blocked` AND `-on-merge` (taskwarrior's exclude-filter syntax, even mid-token), so the tag never lands and the literal `+blocked-on-merge` ends up appended to the description as plain text. The `tags` array stays null and urgency does not tick up. Quoting (`'+blocked-on-merge'`) does **not** help — this is a taskwarrior parser quirk, not a shell issue.

Use underscores or camelCase instead: `+blocked_on_merge`, `+blockedOnMerge`. The example tags above all use underscores deliberately.
