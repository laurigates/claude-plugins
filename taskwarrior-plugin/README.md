...
Hyphens in tag names silently break parsing — `+blocked-on-merge` is read as `+blocked` AND `-on-merge` (the `-` is taskwarrior's exclude-filter syntax) and ends up swallowed into the description with `tags: null`. Quoting does not help. Use underscores or camelCase.
