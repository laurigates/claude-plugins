...
> **Tag naming note:** All tags use underscores rather than hyphens. Hyphens silently break taskwarrior tag parsing — `+blocked-on-merge` is interpreted as `+blocked` AND `-on-merge` (exclude filter), so the tag never lands. See `task-add` for details.
