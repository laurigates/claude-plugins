...
| `/taskwarrior:task-bulk-ops` | Batch operations over many tasks in one pass (queue cleanup, PR-merge sweep, project wrap-up) that dodge the four foot-guns: numeric-ID renumbering (use UUIDs; `+LATEST uuids` not `+LATEST _get`), `task done`/`task config` stdin + confirmation traps (`</dev/null` + `rc.confirmation=no`), filter caveats (`export \| jq` over `list`), and annotate-before-done ordering — see `.claude/rules/task-id-stability.md` for the numeric-ID/UUID discipline that also applies within a single skill's own multi-step lifecycle (`task-claim` / `task-release` / `task-done`), not just bulk loops |
