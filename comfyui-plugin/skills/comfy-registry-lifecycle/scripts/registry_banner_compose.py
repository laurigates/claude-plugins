...
def main():
    ...
    ap.add_argument(
        "--bg", required=True, help="background PNG (registry_banner_bg.py)"
    )
    ap.add_argument(
        "--icon", required=True, help="400x400 icon PNG (cairosvg-rendered)"
    )
    ap.add_argument(
        "--name", required=True, help="DisplayName wordmark, e.g. 'Touch Resize'"
    )
    ap.add_argument("--tagline", required=True, help="one-line tagline")
    ap.add_argument(
        "--accent", default="#6ba6ff", help="tagline colour hex (family accent)"
    )
