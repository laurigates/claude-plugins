...
def build_graph(text, seed, w, h, model):
    return {
        ...
        "vae": {
            ...
            "inputs": {"vae_name": "Flux2/flux2-vae.safetensors"},
        },
        "pos": {
            "class_type": "CLIPTextEncode",
            "inputs": {"clip": ["clip", 0], "text": text},
        },
        "zero": {
        ...
        "guid": {
            ...
            "inputs": {"conditioning": ["pos", 0], "guidance": 4.0},
        },
        "lat": {
        ...
        "save": {
            ...
            "inputs": {"images": ["dec", 0], "filename_prefix": "registry_banner"},
        },
    }
...
def main():
    ...
    ap.add_argument(
        "--accent",
        default="cool blue and azure",
        help='accent phrase, e.g. "warm amber and orange" / "cool blue and azure"',
    )
    ap.add_argument("--seed", type=int, default=1)
    ap.add_argument(
        "--width", type=int, default=1344, help="must be /16; 1344x576 = exact 21:9"
    )
    ap.add_argument("--height", type=int, default=576)
    ap.add_argument(
        "--prompt", default=None, help="override the default background prompt"
    )
    ...
    req = urllib.request.Request(
        f"{a.host}/prompt", data=data, headers={"Content-Type": "application/json"}
    )
    ...
    while time.time() - t0 < 300:
        h = json.loads(
            urllib.request.urlopen(f"{a.host}/history/{pid}", timeout=30).read()
        )
        if pid in h:
            ...
            open(a.out, "wb").write(
                urllib.request.urlopen(f"{a.host}/view?{q}", timeout=60).read()
            )
            print(f"-> {a.out}")
            return
        time.sleep(2)
