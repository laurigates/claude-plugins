...
| `CLAUDE_HOOKS_ENABLE_PROSE_CHECK` | PostToolUse nudge running the house prose rubric over a `.md` file just written, and over a `gh pr create` / `gh issue create` `--body-file`. Augments tool output with the candidate sentences it found; never blocks. Needs the sibling `prose-plugin` for `skills/prose-check/scripts/prose-check.sh` | hooks · `hooks/prose-house-style-nudge.sh` |
...
| `CLAUDE_HOOKS_AUTO_CHECKPOINT_NO_ASTGREP` | Forces auto-checkpoint's no-parser path even when ast-grep is installed, so only the pre-#2652 regex matcher decides — a fail-SAFE fallback that over-checkpoints, where the bash-antipatterns and terraform switches beside it select a fail-open no-op. Its regression suite sets it to pin that polarity (#2652) | hooks · `hooks/auto-checkpoint.sh` |
| `CLAUDE_HOOKS_TERRAFORM_APPLY_NO_ASTGREP` | Forces the terraform apply gate's no-op (ast-grep-absent) path even when ast-grep is installed, so the hook exits 0 on every command. Its regression suite sets it to pin the fail-open half, and asserts the same command blocks without it — the pair is what keeps the suite from passing over a hook that no-ops (#2506) | terraform · `hooks/validate-terraform-apply.sh` |
