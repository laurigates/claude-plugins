...
| `CLAUDE_HOOKS_WORKFLOW_MAX_AGENTS` | `10` | Estimated agent count a Workflow script may reach before the guard asks; the default matches the `medium` `workflowSizeGuideline`. A repetition nothing in the script bounds (`while (true)` whose breaks prove no count, a loop that grows its own list) is costed one over this, so it always asks | `hooks/workflow-scale-guard.sh` |
