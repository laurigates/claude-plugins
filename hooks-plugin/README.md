...
Estimation runs in [`hooks/workflow-scale-estimate.py`](hooks/workflow-scale-estimate.py), which blanks comment and string/template bodies before counting so an `agent(` discussed in a prompt is not read as a call. A template's extent is found structurally, so a template nested inside a `${...}` cannot end the outer literal early. That parse is used only when it proves itself: every literal and comment closes where JS requires, the remaining code's brackets balance, and no `agent(`, fan-out, or `push(` token that the #2668 scan kept as code is blanked. Otherwise the #2668 scan decides. Even a proven parse does not decide alone: those checks cannot see a blanked declaration that bounds a fan-out, so the #2668 scan is costed as well and the higher estimate is reported. The estimate therefore never falls below the #2668 scan's reading of the same script; a false positive the structural parse would have removed stays whenever that scan sees it. The rollup reports which scan decided (`SANITIZER=`, plus `FALLBACK=<reason>`). On a parse that cannot prove itself, and on any file where the estimator finds a regex literal with a quote or backtick in it (a `/` in one of the regex positions the estimator lists: line start, after a punctuator other than `]`, `...`, `*/`, an operand-taking keyword, or a division `/` and whitespace), the #2668 bound logic is costed as well, so there the estimate is never below the #2668 estimator's. Only a proven parse of a file with no such regex can fall below it, through two rules: a site inside a literal-array element runs once, and a trailing comma is not an element. The first applies only when no loop keyword precedes the site in its element and every function enclosing it is the element's own thunk, a modeled `.map` callback, a `pipeline` stage, or a thunk a `.map` returns straight into `parallel()`'s argument list. Methods, getters and class methods (any `{` after a `)` that is not an `if`/`switch`/`catch`/`with` header or a `function`), thunks in other array literals, and thunks indexed out of a `.map` all fail it, because a loop or `.map` after the site can call them. A site that fails is costed as unbounded, never below the literal's length. An array grown by `push` is costed as unbounded, at no less than its initializer's length plus one. Known gaps, all fail-open: `agent()` inside a `${...}` interpolation, fan-out through a helper function, a regex literal whose quote or backtick the #2668 scan misreads too (outside a template, or a backtick inside one), and a quoted regex in a position the estimator's list does not name.
...
**Tests:** `bash hooks-plugin/hooks/test-workflow-scale-guard.sh` (the negatives carry the contract — a guard that asks on ordinary workflows gets disabled within a day). A differential against the frozen #2668 estimator (`hooks/fixtures/workflow-scale-estimate-2668.py`) covers every bundled template: the live estimate may fall below #2668's only to a listed correct count (or, where a literal-array element repeats its site, to a figure at or above the listed true count), never to `NO_AGENTS`, and never on an unproven parse or a file holding a quoted regex literal.
