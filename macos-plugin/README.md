...
| [macos-disk-usage](skills/macos-disk-usage/skill.md) | Disk-usage forensics and space recovery on APFS — trust `df` `Avail` not `Capacity %`, know when `du` over-counts (CoW clones) and under-counts (purgeable snapshots), reclaim OrbStack/Docker, tiered cache cleanup |
