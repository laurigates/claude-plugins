...
if echo "$COMMAND" | grep -q '\-auto-approve'; then
    echo "TERRAFORM SAFETY: 'terraform apply -auto-approve' is blocked.
...
To skip the confirmation prompt after reviewing: terraform apply -auto-approve tfplan" >&2
    exit 2
fi

# Standard terraform apply — gate behind plan review
echo "TERRAFORM SAFETY: Run 'terraform plan' before 'terraform apply'.
...
command referencing the saved plan file." >&2
exit 2
