...
- ~~`cold-read-gate/SKILL.md` retains `run_in_background: false` and `#2063` (the #2063 fix currently has **no** check of its own)~~ — **refuted 2026-07-29.** The #2063 fix *is* guarded: `plugin-compliance-check.sh` (the `cold-read-gate` branch of `check_skill_body()`, currently ~lines 965–971) loops over the tokens `run_in_background: false` and `task-completion result` and errors when either is missing, citing #2063 in both the comment and the failure message; `regression-testing.md` records it as landed. Do **not** add a duplicate check here — that would double-gate (`offload-to-deterministic-substrate.md` §Cautions). If the batch template ships, its new surface is the *verdict enum*, which is what would want a check;
