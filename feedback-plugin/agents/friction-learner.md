...
| `stop:*` / `hook-feedback:*` / `hook-error:*` | **Watch**: report prevalence (`sessions` / `total_sessions`) and same-session repeat (`repeat_sessions` / `sessions`), both rendered as the `Sessions` and `Repeat sessions` columns of the PR-body table (and present in `clusters.json`). A rule file cannot quiet a Stop hook; the fix lives in the emitting hook, so file or cite an issue against it |
