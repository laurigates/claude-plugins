...
| Setting up worktree isolation, or a git write went somewhere unexpected | [`references/worktree-hazards.md`](references/worktree-hazards.md) | cwd-reset guardrail (#1480), `GIT_DIR`-export leak (#1692), nested-repo isolation (#1838), shared scratchpad collisions (#2370), deleted worktree kills the shell (#2372), target-branch preflight (#1969), `isolation: "remote"` resolving to a local worktree (#2447), Workflow agents unreachable + post-run branch pinning (#2614), PR feedback: resume the original worktree agent instead of spawning a fresh one |
| A verify / judge / gate phase runs in the same clone as a builder | [`references/verifier-shared-state.md`](references/verifier-shared-state.md) | Base state from the ref, never the checkout (three-dot diff; say so in the brief); mutating and measuring verifiers never concurrent; restore from a `cp` snapshot, never `git checkout --`; re-run every gate the builder reported |
